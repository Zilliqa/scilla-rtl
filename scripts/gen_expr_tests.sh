...
# and replaces it with a new one by running expr-compiler.
...
if [[ ! -e ${scilla_root}/bin/expr-compiler || ! -d ${testsuite_dir}/expr ]]
then
    echo "${scilla_root}/bin/expr-compiler or ${testsuite_dir}/expr not found"
    exit 1
fi
