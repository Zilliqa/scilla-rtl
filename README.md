...
A custom option `--update-result` has also been provided that will overwrite result files
with the one from the current execution.

For convenience a `CMake` target `check` has been provided to run the testsuite. This can be
executed as `make check` in the build directory.
