...
@_contract_parameters = constant [0 x %"$ParamDescr_111"] zeroinitializer
@_contract_parameters_length = constant i32 0
@_transition_parameters = constant [0 x %"$TransDescr_112"] zeroinitializer
...
"$have_gas_93":                                   ; preds = %"$out_of_gas_92", %"$have_gas_88"
  ...
  %"$two_95" = load %Uint32, %Uint32* %two, align 4
  %"$valueof_96" = extractvalue %Uint32 %"$two_95", 0
  %"$valueof_97" = zext i32 %"$valueof_96" to i64
  %"$gasrem_98" = load i64, i64* @_gasrem, align 8
  %"$gascmp_99" = icmp ugt i64 %"$valueof_97", %"$gasrem_98"
...
"$have_gas_101":                                  ; preds = %"$out_of_gas_100", %"$have_gas_93"
  %"$consume_102" = sub i64 %"$gasrem_98", %"$valueof_97"
  store i64 %"$consume_102", i64* @_gasrem, align 8
  %"$execptr_load_103" = load i8*, i8** @_execptr, align 8
  %"$two_104" = load %Uint32, %Uint32* %two, align 4
  %"$to_nat_call_105" = call %TName_Nat* @_to_nat(i8* %"$execptr_load_103", %Uint32 %"$two_104"), !dbg !12
  store %TName_Nat* %"$to_nat_call_105", %TName_Nat** %"$expr_6", align 8, !dbg !12
  %"$$expr_6_107" = load %TName_Nat*, %TName_Nat** %"$expr_6", align 8
  ret %TName_Nat* %"$$expr_6_107"
}
...
entry:
  %"$exprval_108" = call %TName_Nat* @_scilla_expr_fun(i8* null)
  %"$memvoidcast_109" = bitcast %TName_Nat* %"$exprval_108" to i8*
  %"$execptr_load_110" = load i8*, i8** @_execptr, align 8
  call void @_print_scilla_val(i8* %"$execptr_load_110", %_TyDescrTy_Typ* @"$TyDescr_ADT_Nat_39", i8* %"$memvoidcast_109")
  ret void
}
