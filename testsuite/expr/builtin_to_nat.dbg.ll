...
%Int32 = type { i32 }
%"$ParamDescr_113" = type { %ParamDescrString, %_TyDescrTy_Typ* }
%ParamDescrString = type { i8*, i32 }
%"$TransDescr_114" = type { %ParamDescrString, i32, %"$ParamDescr_113"* }
...
@_contract_parameters = constant [0 x %"$ParamDescr_113"] zeroinitializer
@_contract_parameters_length = constant i32 0
@_transition_parameters = constant [0 x %"$TransDescr_114"] zeroinitializer
...
entry:
  ...
  call void @llvm.dbg.declare(metadata %TName_Nat** %"$expr_6", metadata !11, metadata !DIExpression()), !dbg !14
...
"$have_gas_90":                                   ; preds = %"$out_of_gas_89", %entry
  ...
  call void @llvm.dbg.declare(metadata %Uint32* %two, metadata !15, metadata !DIExpression()), !dbg !17
...
"$have_gas_95":                                   ; preds = %"$out_of_gas_94", %"$have_gas_90"
  ...
  store %Uint32 { i32 2 }, %Uint32* %two, align 4, !dbg !14
  %"$two_97" = load %Uint32, %Uint32* %two, align 4
  %"$valueof_98" = extractvalue %Uint32 %"$two_97", 0
  %"$valueof_99" = zext i32 %"$valueof_98" to i64
  %"$gasrem_100" = load i64, i64* @_gasrem, align 8
  %"$gascmp_101" = icmp ugt i64 %"$valueof_99", %"$gasrem_100"
...
"$have_gas_103":                                  ; preds = %"$out_of_gas_102", %"$have_gas_95"
  %"$consume_104" = sub i64 %"$gasrem_100", %"$valueof_99"
  store i64 %"$consume_104", i64* @_gasrem, align 8
  %"$execptr_load_105" = load i8*, i8** @_execptr, align 8
  %"$two_106" = load %Uint32, %Uint32* %two, align 4
  %"$to_nat_call_107" = call %TName_Nat* @_to_nat(i8* %"$execptr_load_105", %Uint32 %"$two_106"), !dbg !18
  store %TName_Nat* %"$to_nat_call_107", %TName_Nat** %"$expr_6", align 8, !dbg !18
  %"$$expr_6_109" = load %TName_Nat*, %TName_Nat** %"$expr_6", align 8
  ret %TName_Nat* %"$$expr_6_109"
}
...
entry:
  %"$exprval_110" = call %TName_Nat* @_scilla_expr_fun(i8* null)
  %"$memvoidcast_111" = bitcast %TName_Nat* %"$exprval_110" to i8*
  %"$execptr_load_112" = load i8*, i8** @_execptr, align 8
  call void @_print_scilla_val(i8* %"$execptr_load_112", %_TyDescrTy_Typ* @"$TyDescr_ADT_Nat_41", i8* %"$memvoidcast_111")
  ret void
}
...
!2 = !DIFile(filename: "builtin_to_nat.scilexp", directory: "codegen/expr")
...
!12 = !DIDerivedType(tag: DW_TAG_pointer_type, name: "Nat", baseType: !13, size: 8, align: 8, dwarfAddressSpace: 0)
!13 = !DIBasicType(name: "Nat", size: 8)
!14 = !DILocation(line: 1, column: 11, scope: !10)
!15 = !DILocalVariable(name: "two", scope: !10, file: !2, line: 1, type: !16)
