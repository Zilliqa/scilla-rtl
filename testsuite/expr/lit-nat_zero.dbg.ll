...
"$have_gas_90":                                   ; preds = %"$out_of_gas_89", %entry
  ...
  %"$adtval_92" = bitcast i8* %"$adtval_92_salloc" to %CName_Zero*
  %"$adtgep_93" = getelementptr inbounds %CName_Zero, %CName_Zero* %"$adtval_92", i32 0, i32 0
  store i8 0, i8* %"$adtgep_93", align 1
  %"$adtptr_94" = bitcast %CName_Zero* %"$adtval_92" to %TName_Nat*
  store %TName_Nat* %"$adtptr_94", %TName_Nat** %"$expr_6", align 8, !dbg !14
  %"$$expr_6_95" = load %TName_Nat*, %TName_Nat** %"$expr_6", align 8
  ret %TName_Nat* %"$$expr_6_95"
}
...
entry:
  %"$exprval_96" = call %TName_Nat* @_scilla_expr_fun(i8* null)
  %"$memvoidcast_97" = bitcast %TName_Nat* %"$exprval_96" to i8*
  %"$execptr_load_98" = load i8*, i8** @_execptr, align 8
  call void @_print_scilla_val(i8* %"$execptr_load_98", %_TyDescrTy_Typ* @"$TyDescr_ADT_Nat_41", i8* %"$memvoidcast_97")
  ret void
}
